create or replace function public.attach_delivery_files(
  p_delivery_id uuid,
...
begin
  ...
  for v_attachment in select * from jsonb_array_elements(coalesce(p_attachments, '[]'::jsonb)) loop
    ...
    v_size_bytes := case
      when coalesce(v_attachment ->> 'size_bytes', '') ~ '^[0-9]+$' then (v_attachment ->> 'size_bytes')::integer
      else 0
    end;
...
begin
  ...
  if v_client_request_id is not null then
    ...
    if found then
      perform public.attach_delivery_files(v_delivery.id, p_attachments);
...
revoke execute on function public.attach_delivery_files(uuid, jsonb) from anon, authenticated, public;
grant execute on function public.attach_delivery_files(uuid, jsonb) to service_role;
